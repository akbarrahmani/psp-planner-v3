...
RxList<CostCat> wc = costCat.values.toList().obs;
_intPage() {
  wc.value = costCat.values.toList();
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: Obx(() => SingleChildScrollView(
            child: Column(children: wc.map((e) => CostCatItem(e)).toList()))));
  }
}

class CostCatItem extends GetView {
  CostCat item;
  CostCatItem(this.item, {super.key});
          ...
          trailingActions: [
            if (cost.values
              ...
              SwipeAction(
                  ...
                  onTap: (handler) async {
                    handler(false);
                    for (var i = 0; i < costCat.length; i++) {
                      var e = costCat.getAt(i)!;
                      if (e.id == item.id) {
                        await costCat.deleteAt(i);
                        break;
                      }
                    }
                  }),
